...
<script>
import {apiAdminUrl, apiUrl, redirect} from "@/plugins/helpers/misc";
...
export default {
  ...
  data() {
    return {
      ...
      breadcrumbs: [
        ...
        {
          name: 'Reseller Info',
          url: `/admin/resellers/${this.resellerId}`
        },
        {
  ...
  methods: {
    update() {
      ...
      this.xhrPut(apiAdminUrl(`resellers/${this.resellerId}`), this.form)
        .then(async resp => {
          Swal.fire({
            title: 'Reseller',
            ...
            redirect(`/admin/resellers/${resp.data.data['id']}`)
          })
        })
        .finally(() => {
  ...
  mounted() {
    ...
    this.xhrGet(apiAdminUrl(`resellers/${this.resellerId}`)).then(resp => {
