...
<script>
import {apiUrl, isMobileDevice, startWorking, stopWorking} from "~/plugins/helpers/misc";
...
export default {
  ...
  methods: {
    ...
    beneficiaryDeletionConfirmed() {
      ...
      this.xhrDelete(apiUrl(`beneficiaries/${this.beneficiary['beneficiary_id']}`))
        .then(() => {
          Alert.success({
            title: 'Beneficiary',
    ...
    numberDeletionConfirmed() {
      startWorking('Deleting')
      this.xhrDelete(apiUrl(`beneficiaries/${this.beneficiary['beneficiary_id']}/mobile-numbers/${this.current_mobile_number['bmn_id']}`))
        .then(() => {
          this.table.ajax.reload()
    ...
    async fetchInfo() {
      ...
      const resp = await this.xhrGet(apiUrl(`beneficiaries/${this.beneficiary_id}`));
    ...
    fetchMobileNumbers() {
      initDataTable('table#mobile-numbers', {
        url: apiUrl(`beneficiaries/${this.beneficiary_id}/mobile-numbers`),
